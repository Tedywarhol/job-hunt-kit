...
- `.mcp.json` déclare le serveur MCP tiers `linkedin` ([`mcp-server-linkedin`](https://github.com/stickerdaniel/linkedin-mcp-server), optionnel) : `job-scout` l'utilise en lecture seule pour `search_jobs`/`get_job_details`, `recruiter-outreach` pour `search_people`/`get_company_employees`/`get_person_profile`. `send_message` et `connect_with_person` sont exclus des instructions de ces agents — aucun envoi ni action de compte LinkedIn automatisé (cf. README, section LinkedIn).
